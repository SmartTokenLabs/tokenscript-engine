...
class TSTokenView {
    ...
    constructor(element: Element) {
        ...
        for (i in 0 until element.getChildNodes().getLength()) {
            val child: Node? = element.getChildNodes().item(i)
            when (child?.getNodeType()) {
                NodeConsts.ELEMENT_NODE -> when ((child as Element).getLocalName()) {
                    "style" ->                             //record the style for this
                        lStyle += getHTMLContent(child)
                    else -> lView += getElementHTML(child)
                }
                NodeConsts.TEXT_NODE -> if (element.getChildNodes().getLength() == 1) {
    ...
    constructor(style: String, view: String) {
        this.style = style
        tokenView = view
    }

    private fun getElementHTML(content: Node): String {
        val sb: StringBuilder = StringBuilder()
        val localName = (content as Element).getLocalName()
    ...
    private fun getHTMLContent(content: Node): String {
        ...
        for (i in 0 until content.getChildNodes().getLength()) {
            val child: Node? = content.getChildNodes().item(i)
            when (child?.getNodeType()) {
                ...
                else -> if (child != null && child.getTextContent() != null) {
    ...
    private fun htmlAttributes(attribute: Node?): String {
        ...
        if (attribute.getAttributes().getLength() > 0) {
            for (i in 0 until attribute.getAttributes().getLength()) {
                val attr: Attr = attribute.getAttributes().item(i) as Attr
                sb.append(" ")
    ...
    companion object {
        fun getDefaultView(assetDetailViewName: String?): TSTokenView {
            val style =
...
label {
  ...
  margin-top: 6px;
            ...
            val view = """<script type="text/javascript"> //
class Token {
