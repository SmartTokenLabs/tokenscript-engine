...
export type ViewerTypes = 'tabbed' | 'integration' | 'new' | 'joyid-token' | 'marketplace' | 'sts-token' | 'alphawallet';
...
export class AppRoot {
  ...
  constructor() {
    if (this.viewerType !== 'marketplace') dbProvider.checkCompatibility();
    ...
    this.tsEngine = new TokenScriptEngine(
      ...
      {
        noLocalStorage: this.viewerType === 'marketplace',
        trustedKeys: [
          {
  ...
  async getWalletAdapter(): Promise<IWalletAdapter> {
    ...
    } else if (this.viewerType === 'marketplace') {
      providerFactory = async () => {
  ...
  async componentDidLoad() {
    ...
    if (!this.shouldUseIframeProvider() && this.viewerType !== 'marketplace') {
      ...
      <Host>
        <div class="app-container">
          ...
          <main>
            ...
            {this.viewerType === 'marketplace' ? <marketplace-viewer app={this}></marketplace-viewer> : ''}
        ...
        {!this.shouldUseIframeProvider() && this.viewerType !== 'marketplace' ? <wallet-selector ref={el => (this.walletSelector = el)}></wallet-selector> : ''}
      </Host>
    );
  }
}
